...
{	
    ...
    if(err !=0) for(ie=12;ie<=NNUCREAC;ie++)
    {
        ferrhigh[ie]=9.;
        ferrlow[ie]=-0.9;
     }
				...
				if(err !=0)
				{
					if(T9<2.5)
					{
						ferrhigh[24]=sqrt(0.08*0.08+pow((((1.6425644e7-7.682657e8*T9+1.2461811e9*T9*T9-1.1914365e8*pow(T9,1./3.)
														 ...
														 exp(6.34172901*pow(T9,-1./3.))
														 +exp(-1.0418442*T9*T9-5.5570697*pow(T9,-1./3.))*
...
#pragma omp parallel for
    ...
    if(err !=0)
    {
        for(ie=12;ie<=NNUCREAC;ie++)
        {
            ...
            if(err==1) f[ie]*=1.+ferrhigh[ie];
            if(err==2) f[ie]*=1.+ferrlow[ie];
 			if(err==4) 
 			{
		...
		//if(err==3 && paramerror->errnumber>=2 && paramerror->errnumber<=NNUCREAC) f[paramerror->errnumber]*=min(9.,1.+min(UPPERR,ferrhigh[paramerror->errnumber]));
    }
